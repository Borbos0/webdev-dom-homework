import { initEventListeners } from "./render.js";
...
export function renderAuthorizationButton(comments) {
    const authorizationButtonIn = document.getElementById("authorization-button-in");
    ...
    authorizationButtonIn.addEventListener("click", () => {
        ...
        initPostAuth(authorizationLogin.value, authorizationPassword.value).then((responseData) => {
            setToken(responseData.user.token);
            renderComments(comments, responseData.user.name);
        }).catch((error) => {
